...
required_ids = {
    ...
    "downloadTransactionTemplateXlsx", "downloadTransactionTemplateCsv"
}
...
check("Professional-use limitation is visible", "Scope and Limitation Disclosure" in soup.get_text(" ", strip=True))
...
# Version 1.3.5 UX requirements.
check("Version 1.3.5 is configured", "APP_VERSION = '1.3.5'" in js and dataset.get("app_version") == "1.3.5")
...
# Version 1.3.5 transaction-analysis and independently audited measurement-period requirements.
...
check("XLSX and CSV template download controls exist", bool(soup.find(id="downloadTransactionTemplateXlsx") and soup.find(id="downloadTransactionTemplateCsv")))
check("Template downloads are in-browser and not static-path dependent", "TRANSACTION_TEMPLATE_XLSX_BASE64" in js and "downloadTransactionTemplateXlsx" in js and "downloadTransactionTemplateCsv" in js)
...
check("App version 1.3.5 is in dataset", dataset.get("app_version") == "1.3.5")
